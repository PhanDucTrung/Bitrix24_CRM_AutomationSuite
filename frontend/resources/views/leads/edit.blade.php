...
<div class="container mx-auto p-4">
    ...
    <form action="{{ route('leads.update', $lead['ID']) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="TITLE" class="border p-2 w-full" value="{{ $lead['TITLE'] }}" required>
        </div>

        <div>
            <label class="block">Tên</label>
            <input type="text" name="NAME" class="border p-2 w-full" value="{{ $lead['NAME'] }}" required>
        </div>
  <div>
    ...
    <input type="email" name="EMAIL[0][VALUE]" class="border p-2 w-full" 
           value="{{ $lead['EMAIL'][0]['VALUE'] ?? '' }}" required>
    <input type="hidden" name="EMAIL[0][VALUE_TYPE]" value="{{ $lead['EMAIL'][0]['VALUE_TYPE'] ?? 'WORK' }}">
</div>

<div>
    <label class="block">Phone</label>
    <input type="text" name="PHONE[0][VALUE]" class="border p-2 w-full" 
           value="{{ $lead['PHONE'][0]['VALUE'] ?? '' }}" required>
    <input type="hidden" name="PHONE[0][VALUE_TYPE]" value="{{ $lead['PHONE'][0]['VALUE_TYPE'] ?? 'WORK' }}">
</div>
 <div>
            <label class="block">Ghi chú</label>
            <input type="text" name="COMMENTS" class="border p-2 w-full" value="{{ $lead['COMMENTS'] }}" required>
        </div>
